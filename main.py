...
def read_data(datafilepath, maskfilepath, variable_dict):
    ...
    decode_cf = not any(key in filename for key in ["grid_T", "grid_T_sampled", "grid_U", "grid_V"])
...
def apply_metrics_restart(data, mask):
    ...
    metric_functions = {
        ...
        "check_density_computed": lambda d: check_density(get_density(d["temperature"], d["salinity"], get_depth(restart, mask),mask["tmask"])[0]),
        "temperature_500m_30NS_metric": lambda d: temperature_500m_30NS_metric(d["temperature"][0], mask),
        "temperature_BWbox_metric": lambda d: temperature_BWbox_metric(d["temperature"][0], mask),
        "temperature_DWbox_metric": lambda d: temperature_DWbox_metric(d["velocity_zonal"][0], mask),
        "ACC_Drake_metric": lambda d: ACC_Drake_metric(d["velocity_zonal"][0], mask),
        "NASTG_BSF_max": lambda d: NASTG_BSF_max(d["velocity_meridional"][0], d["ssh"], mask),
    }
...
def apply_metrics_grid(data_grid_T, data_grid_T_sampled, data_grid_U, data_grid_V, restart, mask):
    """
    Apply metrics to the dataset and mask.
    ...
    metric_functions = {
        ...
        "temperature_500m_30NS_metric": lambda: temperature_500m_30NS_metric(data_grid_T["temperature"], mask),
        "temperature_BWbox_metric": lambda: temperature_BWbox_metric(data_grid_T["temperature"], mask),
        "temperature_DWbox_metric": lambda: temperature_DWbox_metric(data_grid_U["velocity_zonal"], mask),
        "ACC_Drake_metric": lambda: ACC_Drake_metric(data_grid_U["velocity_zonal"], mask),
        "NASTG_BSF_max": lambda: NASTG_BSF_max(data_grid_V["velocity_meridional"], data_grid_T_sampled["ssh"], mask),
    }
...
if __name__ == "__main__":
    ...
    if not args.restart and not (args.grid_T and args.grid_T_sampled and args.grid_U and args.grid_V):
        print("Error: You must specify either --restart or all three grid files: --grid_T, --grid_U, and --grid_V.")
        parser.print_help()
        sys.exit(1)
  ...
  # Collect files paths based on arguments provided
    ...
    if args.restart and not args.grid_T:
        ...
        restart, mesh_mask = read_data(args.restart, args.mesh_mask, VARIABLE_ALIASES)
        data_grid_T, mesh_mask = read_data(args.grid_T, args.mesh_mask, VARIABLE_ALIASES)
        data_grid_T_sampled, mesh_mask = read_data(args.grid_T_sampled, args.mesh_mask, VARIABLE_ALIASES)
        data_grid_T_sampled["time_counter"] = data_grid_T["time_counter"]
        data_grid_U, mesh_mask = read_data(args.grid_U, args.mesh_mask, VARIABLE_ALIASES)
        data_grid_V, mesh_mask = read_data(args.grid_V, args.mesh_mask, VARIABLE_ALIASES)
        results = apply_metrics_grid(data_grid_T, data_grid_T_sampled, data_grid_U, data_grid_V, restart, mesh_mask)
